...
export const Route = createFileRoute("/login")({
  head: () => ({ meta: [{ title: "Sign in — Aarogya AI" }, { name: "description", content: "Sign in to your Aarogya AI health space." }, { property: "og:title", content: "Sign in — Aarogya AI" }, { property: "og:description", content: "Sign in to your Aarogya AI health space." }, { property: "og:type", content: "website" }, { name: "twitter:card", content: "summary_large_image" }] }),
  component: LoginPage,
});
    ...
    <PublicLayout><main className="grid min-h-screen bg-background pt-20 md:grid-cols-2">
      ...
      <section className="flex items-center justify-center px-6 py-14"><div className="w-full max-w-md">
        <div className="rounded-xl border border-border bg-card p-8 shadow-2xl shadow-background/30">
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-tight">Sign in to Aarogya</h1>
          <p className="mt-2 text-sm text-muted-text">Your private space for clearer health decisions.</p>
          <form className="mt-8 space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="text-right"><a href="#forgot-password" className="text-xs text-muted-text hover:text-teal">Forgot password?</a></div>
            <Button className="btn-primary w-full" disabled={loading}>{loading ? <LoaderCircle className="animate-spin" /> : <ArrowRight />} Sign In</Button>
          </form>
          ...
          <p className="mt-6 text-center text-sm text-muted-text">New to Aarogya? <Link className="text-teal hover:underline" to="/signup">Create an account</Link></p>
        </div>
      </div></section>
    </main></PublicLayout>
  );
